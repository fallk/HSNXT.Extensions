...
namespace HSNXT.Test
{
    ...
    public class GenerateTest
    {
        ...
        {
            var result = Extensions.Generate(1, n => n + 2).TakeWhile(n => n < 10);
        ...
        {
            var result = Extensions.Generate("", s => s + 'a').TakeWhile(s => s.Length < 5);
        ...
        {
            var result = Extensions.Generate(0, BreakingFunc.Of<int, int>()).TakeWhile(n => false);
        ...
        {
            Extensions.GenerateByIndex(BreakingFunc.Of<int, int>());
        }

        [Test]
        public void GenerateByIndex()
        {
            var sequence = Extensions.GenerateByIndex(x => x.ToString()).Take(3);
