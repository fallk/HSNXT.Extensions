...
#pragma warning restore 169
        ...
        {
            Test1(char.MinValue, '[', ']',
                @"[div id=""2"" class=""post""]abcdefghijklmnopqrstuvwxyz 0123456789 `~!@#$%^&*()_+-=[]\{}|;':"",./<>?[/div]",
                "div", @"abcdefghijklmnopqrstuvwxyz 0123456789 `~!@#$%^&*()_+-=[]\{}|;':"",./<>?",
        ...
        {
            Test1('$', '[', ']',
                @"abc 012 `~!@#$%^&*()_+-=[]\{}|;':"",./<>? $[div id=""2"" class=""post""]abcdefghijklmnopqrstuvwxyz 0123456789 `~!@#$%^&*()_+-=[]\{}|;':"",./<>?[/div]abc 012 `~!@#$%^&*()_+-=[]\{}|;':"",./<>? ",
                "div", @"abcdefghijklmnopqrstuvwxyz 0123456789 `~!@#$%^&*()_+-=[]\{}|;':"",./<>?",
                new Dictionary<string, string>()
                {
                    {"id", "2"},
        ...
        {
            TestEscaping('$', '[', ']', "Escaping $$[test/] $[date/] finished", "Escaping $[test/]  finished", 1,
                "date", null,
                new Dictionary<string, string>()
                {
                });
        ...
        {
            TestEscaping('$', '[', ']', @"Escaping $$[test/] $[date format=""MM"" plus=""1""]today[/date] finished",
                "Escaping $[test/]  finished", 1, "date", "today",
                new Dictionary<string, string>()
                {
                    {"format", "MM"},
                    {"plus", "1"}
        ...
        private void TestEscaping(char prefix, char open, char close, string content, string expectedContent,
            int expectedTagCount = 0, string expectedTagName = "", string expectedTagContent = "",
