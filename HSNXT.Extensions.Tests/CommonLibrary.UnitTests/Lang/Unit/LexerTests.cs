...
namespace HSNXT.ComLib.Lang.Tests.Unit
{
    ...
    public class Lexer_Tests
    {
        ...
        {
            CheckRead("var a = true; var age = 32; var text = \"#{a} - #{age} + 1\";", 25, "#{a} - #{age} + 1", 39, 1,
                40, 58);
        }
        ...
        private void CheckToken(List<TokenData> tokens, int index, int tokenIndex, Token token, int tokenKind,
            string text, int lineNum, int lineCharPos, int charPos)
        {
            if (token != null)
        ...
        private void CheckRead(string text, int numTokensToRead, string expectedText, int tokenPos, int tokenLine,
            int tokenCharPos, int lexerPos)
        {
