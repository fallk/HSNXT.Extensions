...
namespace HSNXT.ComLib.Automation
{
    ...
    public class Interpreter
    {
        ...
        private const string RegexForLoop =
            @"\s*for\s*\(\s*var\s+(?<name>\w+)\s*=\s*(?<start>[^;]+);\s*(?<checkname>\w+)\s*(?<checkop>[^\s]+)\s*(?<checkvalue>[^;]+);\s*(?<incrementname>\w+)\s*(?<incrementvalue>[^\)]+)\)\{?";
        ...
        public VariableExpr TokenizeVariable(string line, string pattern)
        {
            ...
            if (!match.Success)
                throw new InvalidOperationException("Invalid format of variable : " + line +
                                                    ". Must be var name = <value> | '<value>' | \"<value>\"");
        ...
        public ForLoopExpression TokenizeForLoop(string line, int lineNumber)
        {
            ...
            if (string.Compare(forloop.Variable, checkName, true) != 0)
                throw new ArgumentException("Invalid for loop : variable " + forloop.Variable + " does not match : " +
                                            checkName);
            ...
            else if (forloop.IncrementOp.Contains("+=") || forloop.IncrementOp.Contains("-="))
            {
                ...
                throw new ArgumentException("Invalid increment value supplied in for loop : " + forloop.IncrementOp +
                                            " at line : " + lineNumber);
        ...
        public FunctionCallExpr TokenizeFunctionCall(string line, int linenumber)
        {
            ...
            if (currentChar == ')')
                return new FunctionCallExpr {Name = funcName, ParamMap = new Dictionary<string, object>()};
            char[] endToken = {' ', ',', ')'};
