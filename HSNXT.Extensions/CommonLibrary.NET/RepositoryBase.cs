...
namespace HSNXT.ComLib.Entities
{
    ...
    public abstract class RepositoryBase<T> : RepositoryQueryable, IRepository<T> where T : class, IEntity
    {
        ...
        /// </summary>
        public virtual Action<IList<T>> OnRowsMappedCallBack
        {
            get => _onRowsMappedHandler;
        ...
        {
            var sql = string.Format("update {0} set {1} = (select {2} from {3} where id = {4}) + {5}", _tableName,
                fieldName, fieldName, _tableName, id);
        ...
        public virtual LookupMulti<T> ToLookUpMulti<Id2>(string propName, string filterName)
        {
            ...
            lookup.Init(all, null, item => item.Id,
                item2 => ReflectionUtils.GetPropertyValue(item2, propName) as string, null);
            return lookup;
        }
