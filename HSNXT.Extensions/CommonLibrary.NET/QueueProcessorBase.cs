...
namespace HSNXT.ComLib.Queue
{
    ...
    public class QueueProcessor<T> : IQueueProcessor<T>
    {
        ...
        public virtual QueueStatus GetStatus()
        {
            ...
            lock (SyncRoot)
            {
                status = new QueueStatus(_state, _queue.Count, _lastProcessDate, _numberOfTimesProcessed,
                    _elapsedTimeSinceLastProcessDate, _totalProcessed, NumberToProcessPerDequeue);
            }
