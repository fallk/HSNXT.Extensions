...
namespace HSNXT.ComLib.LocationSupport
{
    ...
    public class LocationValidator : Validator
    {
        ...
        public LocationValidator(Address address, bool isOnline) : this(Location.StatesLookup, Location.CountriesLookup,
            address, isOnline)
        {
        }
        ...
        protected override bool ValidateInternal(ValidationEvent validationEvent)
        {
            ...
            var validStateCountryCombo =
                LocationHelper.IsValidStateCountryRelation(_stateLookup, _countryLookup, _stateId, _countryId);
            ValidationUtils.Validate(!validStateCountryCombo, results, string.Empty,
                "State and country not a valid combination.");
