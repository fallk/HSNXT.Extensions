...
namespace HSNXT.ComLib.Web.Services.Search
{
    ...
    public class SearchEngine
    {
        ...
        /// <param name="culture">The culture.</param>
        /// <returns></returns>
        public static PagedList<SearchResult> SearchSite(string keywords, string siteToSearch, int pageIndex,
            int recordsPerPage, string culture)
        {
            var req = new SearchRequest
            {
                QueryText = keywords,
    ...
    public class SearchEngineBing : ISearchEngineService
    {
        ...
        public PagedList<SearchResult> Search(SearchRequest request)
        {
            ...
            var completeUri = String.Format(url, Settings.AppId, request.Source.ToString(), query, offset,
