    ...
    public class ImportExportService<T> : IImportExportService<T> where T : class, new()
    {
        ...
        public virtual BoolErrorsItem<IList<T>> Import(IList<T> items)
        {
            if (_importHandler == null)
                return new BoolErrorsItem<IList<T>>(null, false, "Import lamda not initialized.", null);
        ...
        public BoolErrorsItem<IList<T>> Load(IDictionary data, string format)
        {
            ...
            Try.CatchLog(() =>
            {
                ...
                result = new BoolErrorsItem<IList<T>>(results, results != null && results.Count > 0, errors.Message(),
                    errors);
            });
            return result;
        }
        ...
        {
            ToDo.Implement(ToDo.Priority.High, "kishore",
                "Need to provide default export functionality for various formats(xml,csv,ini).", () =>
                {
        ...
        public void SetHandlers(Action<IList<T>> importHandler, Func<int, int, IList<T>> exportByPageHandler,
            Func<IList<T>> exportAllHandler, Func<int> totalExportableHandler)
        {
