...
namespace HSNXT.ComLib
{
    ...
    {
        private static readonly IDictionary<string, SimpleStateMap>
            _stateMap = new Dictionary<string, SimpleStateMap>();
        ...
        public static void LoadState(object objToSave, string state, string includeProps, string excludeProps,
            string stringClobProps)
        {
            if (objToSave == null) throw new ArgumentNullException("Object to save is null");
            if (string.IsNullOrEmpty(includeProps) && string.IsNullOrEmpty(includeProps) &&
                string.IsNullOrEmpty(includeProps))
                return;
            ...
            {
                propStateMap =
                    SimpleStateMap.ToPropStateMap(objToSave.GetType(), includeProps, excludeProps, stringClobProps);
        ...
        public static string CreateState(object objToSave, string includeProps, string excludeProps,
            string stringClobProps)
        {
            if (objToSave == null) throw new ArgumentNullException("Object to save is null");
            if (string.IsNullOrEmpty(includeProps) && string.IsNullOrEmpty(includeProps) &&
    ...
    public class SimpleStateMap
    {
        ...
        public static SimpleStateMap ToPropStateMap(Type type, string includePropsDelimeted,
            string excludePropsDelimted, string stringClobPropsDelimited)
        {
            // 1. Map of columns to include.
            var colsToInclude = string.IsNullOrEmpty(includePropsDelimeted)
                ? new string[0]
                : includePropsDelimeted.Split(',');
            var colsToIncludeMap = colsToInclude.ToSameDictionary();

            // 2. Map of columns to exclude.
            var colsToExclude = string.IsNullOrEmpty(excludePropsDelimted)
                ? new string[0]
                : excludePropsDelimted.Split(',');
            var colsToExcludeMap = colsToExclude.ToSameDictionary();

            // 2. Get map of string clob props.
            var stringClobProps = string.IsNullOrEmpty(stringClobPropsDelimited)
                ? new string[0]
                : stringClobPropsDelimited.Split(',');
            ...
            return new SimpleStateMap
            {
                ExcludeProps = colsToExcludeMap,
                IncludeProps = colsToIncludeMap,
                StringClobProps = stringClobMap,
                Props = props,
                AllPropMap = propMap
            };
        }
