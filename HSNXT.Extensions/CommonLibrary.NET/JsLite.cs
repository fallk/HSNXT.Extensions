...
namespace HSNXT.ComLib.Automation
{
    ...
    public class JsLite
    {
        ...
        private const string RegexForLoop =
            @"\s*for\s*\(\s*var\s+(?<name>\w+)\s*=\s*(?<start>[^;]+);\s*(?<checkname>\w+)\s*(?<checkcondition>[^\s]+)\s*(?<checkvalue>[^;]+);\s*(?<incrementname>\w+)\s*(?<incrementval>[^\)]+)\)\{?";
        ...
        public void Interpret(string script)
        {
            ...
            for (var ndx = 0; ndx < _lines.Count; ndx++)
            {
                ...
                else if (tokenType == JsLiteToken.FuncCall)
                    ProcessFuncCall();
                else if (tokenType == JsLiteToken.Var)
                    ProcessVariable();
                else if (tokenType == JsLiteToken.ForLoop)
                    ProcessForLoop();
            }
        }
        ...
        private void ProcessVariable()
        {
            ...
            _visitor(new JsLiteArgs
            {
                LineNumber = _currentLineIndex,
                TokenType = JsLiteToken.Var,
                Name = exp.Name,
                Context = exp.Value
            });
        }
        ...
        private void ProcessFuncCall()
        {
            ...
            var proceed = _visitor(new JsLiteArgs
            {
                LineNumber = _currentLineIndex,
                TokenType = JsLiteToken.FuncCall,
                Name = exp.Name,
                ParamMap = exp.ParamMap,
