    ...
    public class Parser : ParserBase, ILangParser
    {
        ...
        public Expr ParseStatement()
        {
            ...
            while (stmt == null && !_tokenIt.IsEnded)
            {
                ...
                // Token replacements                
                if (hasTokenReplacePlugins && (nexttoken.Kind == TokenKind.Ident) &&
                    _context.Plugins.CanHandleTok(nexttoken, true))
                {
        ...
        /// <returns></returns>
        public Expr ParseExpression(IDictionary<Token, bool> endTokens, bool handleMathOperator = true,
            bool handleSingleExpression = false, bool enablePlugins = true, bool passNewLine = true,
            bool enableIdentTokenTextAsEndToken = false)
        {
            ...
            while (true)
            {
                ...
                // Token replacements                
                if (okToCheckPlugins && enablePlugins && hasTokenReplacePlugins &&
                ...
                if (enableIdentTokenTextAsEndToken && next.Token.Kind == TokenKind.Ident &&
                    identEndTokens.ContainsKey(next.Token.Text))
                ...
                // 2. shunting yard algorithm: "result = a * ( b + 3 )"
                                                        ...
                                                        Terminators.ExpMathShuntingYard.ContainsKey(token))
                {
                    if (exp != null) SetScriptPosition(exp, tokenData);
                    var result = ParseExpressionsWithPrecedence(endTokens, exp, true, identEndTokens,
                        enableIdentTokenTextAsEndToken);
                    exp = result.Item2;
        ...
        /// <returns></returns>
        public ConditionalBlockExpr ParseConditionalStatement(ConditionalBlockExpr stmt,
            string textToEndCondition = "then")
        {
        ...
        */
        ...
        private Tuple<bool, Expr> ParseExpressionsWithPrecedence(IDictionary<Token, bool> endTokens, Expr initial,
            bool enableTokenPlugins = true, IDictionary<string, bool> identTokens = null,
            bool enableIdentTokensAsEndTokens = false)
        {
            ...
            while ((Terminators.ExpMathShuntingYard.ContainsKey(token) || continueParsing) && !_tokenIt.IsEnded)
            {
                ...
                else if (isOp)
                {
                    ...
                    else if (token == Tokens.RightParenthesis)
                    {
                        // Invalid.
                        if (leftParenCount == 0)
                            throw _tokenIt.BuildSyntaxExpectedException(Tokens.LeftParenthesis.Text);
                ...
                // Token replacements                
                if (current != Tokens.EndToken && enableTokenPlugins && hasTokenReplacePlugins &&
                    _context.Plugins.CanHandleTok(current, false))
                {
        ...
        private bool IsEndOfExpressionPart(Expr lastExp, IDictionary<Token, bool> endTokens,
            IDictionary<string, bool> identEndTokens)
        {
            // End of script ?
            var token = _tokenIt.NextToken.Token;
        ...
        private bool IsEndOfExpressionPrecedence(Token token, int leftParenCount, bool continueParsing,
            Expr lastExpression)
        {
        ...
        public Expr ParseFuncExpression(Expr nameExp, List<string> members)
        {
            ...
            else if (!hasMemberAccess)
            {
                ...
                FluentHelper.ParseFuncParameters(funcExp.ParamListExpressions, _tokenIt, this, expectParenthesis,
                    !expectParenthesis, meta);
            }
        ...
        public Expr ParseIdExpression(string name = null, Expr existing = null, bool isCurrentTokenAMember = false)
        {
            ...
            var isMemberAccessAhead = (aheadToken.Token == Tokens.LeftParenthesis ||
                                       aheadToken.Token == Tokens.LeftBracket || aheadToken.Token == Tokens.Dot);
        ...
        public void ParseParameters(IParameterExpression pexp, bool expectParenthesis, bool advanceAfter,
            bool enableNewLineAsEnd = false)
        {
        ...
        public bool IsEndOfParameterList(Token endToken, bool enableNewLineAsEnd)
        {
            ...
            if (enableNewLineAsEnd)
                if (last != Tokens.Comma && next == Tokens.NewLine)
                    return true;
        ...
        public bool IsExplicitIdentQualifierExpression(Token token)
        {
            ...
            if (isVar && (ahead.Token == Tokens.Dot || ahead.Token == Tokens.LeftBracket ||
                          ahead.Token == Tokens.Assignment))
                return true;
        ...
        protected bool IsIncrementOp(Token token)
        {
            if (token == Tokens.Increment || token == Tokens.Decrement || token == Tokens.IncrementAdd ||
                token == Tokens.IncrementDivide || token == Tokens.IncrementMultiply ||
                token == Tokens.IncrementSubtract)
                return true;
            return false;
        }
        ...
        /// <returns></returns>
        protected Expr ParsePostfix(Expr exp, bool enablePlugins, bool hasTokenReplacePlugins,
            bool isCurrentToken = false)
        {
