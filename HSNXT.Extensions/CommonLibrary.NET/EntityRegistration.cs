...
namespace HSNXT.ComLib.Entities
{
    ...
    public class EntityRegistration
    {
        ...
        {
            Register(() => new EntityService<T>(), () => repo, () => new EntityValidator(), null, null,
                configureRepository, connId);
        }
        ...
        {
            Register(() => new EntityService<T>(), repoCreator, () => new EntityValidator(), null, null,
                configureRepository, null);
        }
        ...
        /// <param name="configureRepository">Whether or not to configure the reposiory.</param>
        /// <param name="connId"></param>
        public static void Register<T>(Func<IRepository<T>> repoCreator, bool configureRepository, string connId)
            where T : IEntity
        {
            Register(() => new EntityService<T>(), repoCreator, () => new EntityValidator(), null, null,
                configureRepository, connId);
        ...
        public static void Register<T>(Func<IRepository<T>> repoCreator,
            Func<IEntityValidator> validatorCreator, IEntitySettings settings, IEntityResources resources,
            bool configureRepository, string connId) where T : IEntity
        {
            Register(() => new EntityService<T>(), repoCreator, validatorCreator, settings, resources,
                configureRepository, connId);
        }
        ...
        public static void Register<T>(Func<IEntityService<T>> serviceCreator, Func<IRepository<T>> repoCreator,
            Func<IEntityValidator> validatorCreator, IEntitySettings settings, IEntityResources resources,
            bool configureRepository, string connectionId)
        {
            var ctx = new EntityRegistrationContext();
            ctx.EntityType = typeof(T);
            ctx.Name = typeof(T).FullName;
        ...
        public static object GetService(string typeFullName)
        {
            if (!_managableEntities.ContainsKey(typeFullName))
                throw new ArgumentException("The type : " + typeFullName +
                                            " has not been registered in EntityRegistration");
