...
namespace HSNXT.ComLib.MapperSupport
{
    ...
    public class MapperIni<T> : IMapper<T> where T : class, new()
    {
        ...
        /// <returns></returns>
        public IList<T> Map(object source, IErrors errors, bool enableDynamicTypes, string dynamicTypeSectionKeyName,
            Func<string, T> dynamicTypeFactory)
        {
            if (source == null || !(source is IDictionary))
                return new List<T>();
        ...
        public IList<T> Map(IErrors errors)
        {
            ...
            var counter = 0;
            var propMapDefault =
                ReflectionUtils.GetPropertiesAsMap<T>(
        ...
        /// <returns></returns>
        private Tuple2<T, IDictionary<string, PropertyInfo>> GetNewObjectAndPropertyMapAsTuple(IDictionary section,
            IDictionary<string, PropertyInfo> propMapDefault)
        {
            T item = null;
            ...
            if (_dynamicTypesEnabled)
            {
                ...
                propMap = ReflectionUtils.GetPropertiesAsMap(item.GetType(),
                    BindingFlags.Instance | BindingFlags.Public | BindingFlags.SetProperty, false);
            }
            else
            {
