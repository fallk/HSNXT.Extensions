...
namespace HSNXT.ComLib.LocationSupport
{
    ...
    public class LocationService : ILocationService
    {
        ...
        public LocationService(Func<IRepository<Country>> countryRepoGetter, Func<IRepository<State>> stateRepoGetter,
            Func<IRepository<City>> cityRepoGetter)
        {
        ...
        public LocationService(IRepository<Country> countryRepo, IRepository<State> stateRepo,
        ...
        public CityLookUp CitiesLookup
        {
            get
            {
                return GetCachedLookup("cities_lookup", _settings.CacheCityLookup,
                    _settings.CacheTimeForCitiesInSeconds, () => new CityLookUp(Cities.GetAll()),
                    lookup => lookup == null || lookup.Lookup1.Count == 0);
            }
        }
        ...
        public IList<City> CitiesList
        {
            get
            {
                return GetCached("cities_list", _settings.CacheCityList, _settings.CacheTimeForCitiesInSeconds,
                    () => Cities.GetAll());
            }
        }
        ...
        public CountryLookUp CountriesLookup
        {
            get
            {
                return GetCachedLookup("countries_lookup", _settings.CacheCountryLookup,
                    _settings.CacheTimeForCountriesInSeconds, () => new CountryLookUp(Countries.GetAll()),
                    lookup => lookup == null || lookup.Lookup1.Count == 0);
            }
        }
        ...
        public IList<Country> CountriesList
        {
            get
            {
                return GetCached("countries_list", _settings.CacheCountryList, _settings.CacheTimeForCountriesInSeconds,
                    () => Countries.GetAll());
            }
        }
        ...
        public StateLookUp StatesLookup
        {
            get
            {
                return GetCachedLookup("states_lookup", _settings.CacheStateLookup,
                    _settings.CacheTimeForStatesInSeconds, () => new StateLookUp(States.GetAll()),
                    lookup => lookup == null || lookup.Lookup1.Count == 0);
            }
        }
        ...
        public IList<State> StatesList
        {
            get
            {
                return GetCached("states_list", _settings.CacheStateList, _settings.CacheTimeForStatesInSeconds,
                    () => States.GetAll());
            }
        }
        ...
        public BoolMessageItem<Country> Country(string name)
        {
            ...
            var countryLookup = Cacher.Get("countries_lookup", 300,
                () => Location.Countries.ToLookUpMulti<string>("Name"));
        ...
        public BoolMessageItem<IList<State>> StatesFor(string countryname)
        {
            ...
            var states = Cacher.Get<IList<State>>("states_list_for_" + country.RealId, 300,
                () => Location.States.GetAll().Where(s => s.CountryId == country.RealId && !s.IsAlias && s.IsActive)
                    .ToList());
        ...
        public BoolMessageItem<IList<City>> CitiesFor(string countryname, string statename)
        {
            ...
            var country = countryResult.Item;
            var states = Cacher.Get("states_lookup_for_" + country.RealId, 300,
                () => new StateLookUp(Location.States.GetAll()
                    .Where(s => s.CountryId == country.RealId && !s.IsAlias && s.IsActive).ToList()));
            var state = states[statename];
            if (state == null)
                return new BoolMessageItem<IList<City>>(null, false, "Unknown state supplied.");

            var cities = Cacher.Get<IList<City>>($"cities_for_{country.RealId}_{state.RealId}", 300,
                () => Location.Cities.GetAll().Where(c =>
                    c.CountryId == country.RealId && c.StateId == state.RealId && !c.IsAlias && c.IsActive).ToList());
            var success = cities != null && cities.Count > 0;
            var message = success
                ? string.Empty
                : "Cities not available for country : " + country.Name + ", state : " + state.Name;
        ...
        private LocationLookUpResult InternalParse(string locationData)
        {
            ...
            try
            {
                ...
                if (containsComma)
                {
                    return LocationParser.ParseCityWithStateOrCountry(cityLookUp, stateLookUp, countryLookUp,
                        locationData);
                }
        ...
        private LocationLookUpResult ParseUnitedStatesZipCode(string zip)
        {
            ...
            if (zip.Length == 5)
            {
                for (var ndx = 0; ndx < zip.Length; ndx++)
                {
                    ...
                    {
                        result = new LocationLookUpResult(LocationLookUpType.Zip, false,
                            "U.S. Zip code format is not valid.");
                        result.Zip = zip;
