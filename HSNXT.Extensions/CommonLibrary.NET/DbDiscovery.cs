...
namespace HSNXT.ComLib.Data
{
    ...
    public class DBSchema : Database
    {
        ...
        public string GetDropTable(string tableName, bool includeGo)
        {
            ...
            var checkDelete = "IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[" +
        ...
        public string GetDropProc(string tableName, string procName)
        {
            // IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Categories]') AND type in (N'U'))
            // DROP TABLE [dbo].[Categories]
            var dropCommand = "IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[" +
                              tableName + "_" + procName + "]') AND type in (N'P', N'PC'))" + Environment.NewLine
