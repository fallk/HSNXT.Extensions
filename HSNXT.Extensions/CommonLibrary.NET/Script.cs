...
namespace HSNXT.ComLib.Automation
{
    ...
    public abstract class Script
    {
        ...
        public ScriptResult Run(string content)
        {
            ...
            var error =
                "Assemblies for commands not specified. 'Assemblies' property must be set with delimited list of assemblies containing commands";
        ...
        /// <param name="content"></param>
        /// <returns></returns>
        protected virtual BoolMessage DoRun(string content)
        {
        ...
        /// <returns></returns>
        protected CommandResult ExecuteCommand(string commandName, List<object> paramList, object paramMap,
            int commandIndex, Action<Command> autoMapCallback)
        {
            // Valid command ?
            ...
            try
            {
                ...
                {
                    Name = commandName,
                    IsMethodCall = cmdValidationResult.IsMethodCall,
                    ParamList = paramList,
                    ParamMap = paramMap,
                    Meta = commandAttribute
                });
            }
            catch (Exception ex)
            {
                result = new BoolMessageItem(null, false,
                    "Error executing command : " + commandName + ". " + ex.Message);
            }
        ...
        private CommandValidationResult ValidateCommandCall(string commandName, int commandIndex)
        {
            ...
            {
                var errorResult =
                    CommandHelper.BuildUnKnownCommandResult(commandName, commandIndex, commandCreateResult.Message);
            ...
            {
                var errorMethodCommand =
                    CommandHelper.BuildMethodCallNotSupportedResult(actualCommandName, commandIndex);
                return new CommandValidationResult
                {
                    Success = false,
                    Result = errorMethodCommand,
                    ActualCommandName = actualCommandName,
                    IsMethodCall = isMethodCall
                };
            }
