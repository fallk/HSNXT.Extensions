...
namespace HSNXT.ComLib.CsvParse
{
    ...
    public class CsvDoc
    {
        ...
        public CsvDoc(string contentOrFilePath, bool isFilePath, CsvConfig settings, bool autoLoad)
        {
            ...
            var lexListSettings =
                new LexListSettings {MultipleRecordsUsingNewLine = true, Delimeter = settings.Separator};
        ...
        public void ForEach<T>(int column, int startingRow, int endingRow, Action<int, int, T> action)
        {
            if (column < 0 || column >= Columns.Count)
                throw new IndexOutOfRangeException("Column index out of range : " + column);
            if (startingRow < 0 || startingRow >= Data.Count)
                throw new IndexOutOfRangeException("StartRow index out of range : " + startingRow);
            if (endingRow < 0 || endingRow < startingRow || endingRow >= Data.Count)
                throw new IndexOutOfRangeException("EndRow index out of range : " + endingRow);
        ...
        private void ParseDict()
        {
            ...
            for (var ndx = startingDataRecord; ndx < records.Count; ndx++)
            {
                ...
                if (record.Count != columnNames.Count)
                    throw new ArgumentException("Record at line : " + (ndx + 1) +
                                                " does not have same number of columns as in header.");
        ...
        public void Write(string fileName, string delimeter, bool quoteAll, string quoteChar, string newLine,
            bool append)
        {
