...
namespace HSNXT.ComLib
{
    ...
    public class Scanner
    {
        ...
        public bool ConsumeUntil(string pattern, bool includePatternInConsumption, bool moveToStartOfPattern,
            bool movePastEndOfPattern)
        {
        ...
        public ScanTokenResult ReadCodeString(char quoteChar, char escapeChar = '\\', bool advanceFirst = true,
            bool setPosAfterToken = true)
        {
        ...
        public ScanTokenResult ReadNumber(bool advanceFirst, bool setPosAfterToken = true)
        {
            var sign = "";
            if (advanceFirst) ReadChar();
            if (_state.CurrentChar == '+' || _state.CurrentChar == '-')
            {
                sign = _state.CurrentChar.ToString();
                ReadChar();
            }
        ...
        /// <param name="setPosAfterToken">True to move position to end quote, otherwise past end quote.</param>
        /// <returns>Contents of token read.</returns>
        public ScanTokenResult ReadString(char quoteChar, char escapeChar = '\\', bool advanceFirst = true,
            bool setPosAfterToken = true)
        {
            // "name" 'name' "name\"s" 'name\'"
            var buffer = new StringBuilder();
        ...
        /// <returns></returns>
        public string ReadTokenUntil(char[] endChars, bool includeEndChar = false, bool advanceFirst = false,
            bool readPastEndChar = false)
        {
            var buffer = new StringBuilder();
            var found = false;
            if (advanceFirst) ReadChar();
        ...
        public string ReadToken(char endChar, char escapeChar, bool includeEndChar, bool advanceFirst,
            bool expectEndChar, bool readPastEndChar)
        {
            var buffer = new StringBuilder();
            var currentChar = advanceFirst ? ReadChar() : _state.CurrentChar;
        ...
        {
            return ReadChars(() => _state.CurrentChar != ' ' && _state.CurrentChar != '\t', advanceFirst,
                setPosAfterToken);
        }
        ...
        /// <returns></returns>
        public ScanTokenResult ReadWordWithExtraChars(bool advanceFirst, bool setPosAfterToken = true,
            char extra1 = char.MinValue, char extra2 = char.MinValue)
        {
            // while for function
            var buffer = new StringBuilder();
            if (advanceFirst) ReadChar();
        ...
        /// <returns>String read.</returns>
        public ScanTokenResult ReadLinesUntilChars(bool advanceFirst, char first, char second,
            bool moveToEndChar = true)
        {
            var lineCount = 0;
            return ReadChars(() =>
            {
        ...
        /// <param name="readPastEndChar">True to read past the end character.</param>
        /// <returns>Contents of token read.</returns>
        public string ReadTokenUntil(char endChar, char escapeChar, bool advanceFirst, bool expectEndChar,
            bool includeEndChar, bool moveToEndChar, bool readPastEndChar)
        {
            // abcd <div>
            var buffer = new StringBuilder();
            var currentChar = advanceFirst ? ReadChar() : _state.CurrentChar;
        ...
        internal void SetPosition(int pos)
        {
            ...
            if (pos >= LAST_POSITION)
                throw new LangException("Lexical Error", "Can not set position to : " + pos, "", -1, -1);
        ...
        /// <param name="setPosAfterToken">True to move position to end space, otherwise past end space.</param>
        /// <returns>Contents of token read.</returns>
        public ScanTokenResult ReadChars(IDictionary<char, bool> validChars, bool advanceFirst,
            bool setPosAfterToken = true)
        {
            // while for function
            var buffer = new StringBuilder();
