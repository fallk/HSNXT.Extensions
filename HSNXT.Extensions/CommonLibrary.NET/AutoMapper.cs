...
namespace HSNXT.ComLib
{
    ...
    public class AutoMapper
    {
        ...
        /// <returns></returns>
        public static T Map<T>(IDictionary source, T item, AutoMapperSettings settings, IList<string> errors)
            where T : class
        {
            // Map of properties
        ...
        /// <param name="errors"></param>
        /// <returns></returns>
        public static T Map<T>(XmlNode source, T item, AutoMapperSettings settings, IList<string> errors)
            where T : class
        {
            // Map of properties
            var propMap = AutoMapperHelper.GetPropertyMap(item.GetType(), settings.IsCaseSensitive);
            ...
            for (var ndx = 0; ndx < source.Attributes.Count; ndx++)
            {
                ...
                if (!hasNameFilter || (hasNameFilter && key.StartsWith(nameFilter)))
                {
                    ...
                    {
                        var nested = AutoMapperHelper.GetNestedProperty(item, source.Attributes[ndx].Name,
                            settings.AutoCreateNestedObjects);
                        SetValue(nested.Value, settings, nested.Key, nested.Value.Name, value, errors);
                    }
                }
            }
            ...
            object item, string destinationKeyName, object value, IList<string> errors)
        {
            ...
            if (settings.CatchErrors)
                ...
                {
                    if (errors != null)
                        errors.Add("Unable to map source " + destinationKeyName + " to destination " +
                                   destinationKeyName);
                }
            else
            {
