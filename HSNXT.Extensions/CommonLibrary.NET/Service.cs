...
namespace HSNXT.ComLib.Entities
{
    ...
    public class EntityService<T> : IEntityService<T> where T : IEntity
    {
        ...
        protected virtual BoolResult<T> PerformAction(IActionContext ctx, Action<IActionContext> action,
        ...
        protected virtual BoolResult<T> PerformAuthentication(IActionContext ctx, EntityAction action)
        {
            ...
            if (ctx.Item != null && ctx.Item is IEntity && action == EntityAction.Update ||
                action == EntityAction.Delete)
            {
                ...
                if (!entity.IsOwnerOrAdmin())
                {
                    ...
                    return new BoolResult<T>(default, false,
                        "User : " + Auth.UserName + " is not authorized to edit / delete this item.", ctx.Errors);
                }
            }
