...
namespace HSNXT.ComLib.Lang.Parsing
{
    ...
    public class TokenIterator
    {
        ...
        */
        ...
        {
            if (NextToken.Token == Tokens.EndToken) throw BuildEndOfScriptException();
            if (!allowLiteralAsId && !(NextToken.Token.Kind == TokenKind.Ident))
                throw BuildSyntaxExpectedException("identifier");
        ...
        /// e.g. [ "refill", "refill inventory" ]
        /// </summary>
        public List<Tuple<string, int>> PeekConsequetiveIdsAppendedWithTokenCounts(
            bool enableCamelCasingAsSeparateWords, int maxTokenLookAhead)
        {
            var total = 1;
            var currentWord = NextToken.Token.Text;
            var ahead = Peek(1, false);
        ...
        {
            return new LangException("Syntax Error", $"Expected {expected} but found '{NextToken.Token.Text}'",
                ScriptPath, NextToken.Line, NextToken.LineCharPos);
        }
        ...
        {
            return new LangException("Syntax Error", $"Expected {token.Text} but found '{NextToken.Token.Text}'",
                ScriptPath, NextToken.Line, NextToken.LineCharPos);
        }
        ...
        {
            return new LangException("Syntax Error", $"Unexpected token found '{NextToken.Token.Text}'", ScriptPath,
                NextToken.Line, NextToken.LineCharPos);
        }
        ...
        {
            return new LangException("Syntax Error", $"Unexpected token found '{token.Token.Text}'", ScriptPath,
                token.Line, token.LineCharPos);
        }
        ...
        {
            return new LangException("Syntax Error", $"Unexpected token found {unexpectedTokenText}", ScriptPath,
                token.Line, token.LineCharPos);
        }
        ...
        {
            return new LangException("Syntax Error", "Unexpected end of script", ScriptPath, NextToken.Line,
                NextToken.LineCharPos);
        }
