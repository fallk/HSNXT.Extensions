...
namespace HSNXT.ComLib.Lang.Helpers
{
    ...
    public class MemberHelper
    {
        ...
        {
            var isVariableExp = varExp is VariableExpr;
            ...
            if (isVariableExp)
            {
                var result = IsExternalTypeName(ctx.Memory, ctx.Types, variableName);
                if (result.Success)
                    return GetExternalTypeMember(node, (Type) result.Item, variableName, null, memberName, true);
            }
            ...
            if (isCoreType)
            {
                var result = GetLangBasicTypeMember(node, ctx.Methods, obj, memberName);
                return result;
            }
            ...
            var member =
                GetExternalTypeMember(node, lclassType.DataType, variableName, lclass.Value, memberName, false);
            return member;
        }
        ...
        /// <returns></returns>
        public static MemberAccess GetLangBasicTypeMember(AstNode node, RegisteredMethods methods, LObject obj,
            string memberName)
        {
            var type = obj.Type;
            ...
            if (!typeMethods.HasMember(obj, memberName))
                throw ExceptionHelper.BuildRunTimeException(node,
                    "Property or Member : " + memberName + " does not exist");
        ...
        /// <returns></returns>
        public static MemberAccess GetExternalTypeMember(AstNode node, Type type, string varName, object obj,
            string memberName, bool isStatic)
        {
