...
namespace HSNXT.ComLib.Lang.Parsing
{
    internal class Limits
    {
        ...
        internal void CheckScopeStringLength(AstNode node)
        {
            if (_hasSettings && _ctx.Settings.MaxScopeStringVariablesLength > 0)
            {
                ...
                if (_ctx.Memory.TotalStringLength > _ctx.Settings.MaxScopeStringVariablesLength)
                    throw BuildLimitException(node, "total string variables length",
                        _ctx.Settings.MaxScopeStringVariablesLength);
            }
        }
        ...
        internal void CheckParserStatement(AstNode node)
        {
            ...
            if (_hasSettings && _ctx.Settings.MaxStatements > 0 &&
                _ctx.State.StatementCount >= _ctx.Settings.MaxStatements)
        ...
        {
            if (_hasSettings && _ctx.Settings.MaxStatementsNested > 0 &&
                statementNestedCount > _ctx.Settings.MaxStatementsNested)
        ...
        {
            if (_hasSettings && _ctx.Settings.MaxFuncCallNested > 0 &&
                functionCallCount > _ctx.Settings.MaxFuncCallNested)
        ...
        {
            if (_hasSettings && _ctx.Settings.MaxConsequetiveExpressions > 0 &&
                expressionCount > _ctx.Settings.MaxConsequetiveExpressions)
                throw BuildLimitException(node, "consequetive expressions (" + expressionCount + ")",
        ...
        {
            if (_hasSettings && _ctx.Settings.MaxConsequetiveMemberAccess > 0 &&
                memberAccessCount > _ctx.Settings.MaxConsequetiveMemberAccess)
                throw BuildLimitException(node, "consequetive member access ( '(),[],.' )",
                    _ctx.Settings.MaxConsequetiveMemberAccess);
        }
