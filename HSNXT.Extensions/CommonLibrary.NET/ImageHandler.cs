...
namespace HSNXT.ComLib.Web.HttpHandlers
{
    ...
    public class ImageHandler : HandlerBase, IHttpHandler
    {
        ...
        public static void InitExternalHandler(Func<HttpContext, bool> externalImageHandlerCheck,
            Action<HttpContext> externalImageHandler)
        {
        ...
        public override void ProcessRequest(HttpContext context)
        {
            ...
            {
                if (_enableLogging)
                    Logger.Debug("CommonLibrary ImageHandler ProcessRequest using external handler for : " +
            ...
            try
            {
                ...
                if (_enableLogging)
                    Logger.Debug("CommonLibrary ImageHandler Processing image : " + fileName + ", physical path : " +
                                 physicalpath);
                if (fi.Exists)
                {
                    ...
                    if (_enableLogging)
                        Logger.Debug("Image file exists : " + fileName + ", physical file : " + physicalpath);
                }
                else
                {
                    if (_enableLogging)
                        Logger.Debug(
                            "CommonLibrary ImageHandler ProcessRequest ImageFile : " + fileName + " not found.");
            ...
            catch (Exception ex)
            {
                if (_enableLogging)
                    Logger.Debug(
                        "CommonLibrary ImageHandler ProcessRequest : unable to process request for file : " + fileName,
                        ex);
                context.Response.Redirect("/NotFound");
            }
        }
