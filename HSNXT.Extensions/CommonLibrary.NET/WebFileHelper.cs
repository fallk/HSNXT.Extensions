...
namespace HSNXT.ComLib.Web.Helpers
{
    ...
    public class WebFileHelper
    {
        ...
        /// <returns>True if media files were uploaded.</returns>
        public static bool ValidateMediaFiles(HttpRequestBase request, int maxSizeInK, int maxNumFiles,
            Action<string, string> errorStorage)
        {
            // No files ?
            if (request.Files == null || request.Files.Count == 0)
                return true;
            ...
            for (var ndx = 0; ndx < request.Files.Count; ndx++)
            {
                ...
                {
                    if (hasErrorStorage)
                        errorStorage("File : " + (ndx + 1),
                            "File " + name + " with size " + size + " exceeds maximum size of " + maxSizeInK +
                            " kilobytes");
