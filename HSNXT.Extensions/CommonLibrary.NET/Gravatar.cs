...
namespace HSNXT.ComLib.Web.Services.GravatarSupport
{
    ...
    public class Gravatar
    {
        ...
        public Gravatar(string email, int size, string rating, string iconType, string imageExtension)
        {
            ...
            var icon = string.IsNullOrEmpty(iconType)
                ? IconType.none
                : (IconType) Enum.Parse(typeof(IconType), iconType, true);
        ...
        public string Url
        {
            get
            {
                ...
                {
                    finalUrl = string.Format(URL_WITH_ICON, emailHash, ImageExtension, IconType.ToString(), Size,
                        Rating.ToString());
                }
                else
                {
