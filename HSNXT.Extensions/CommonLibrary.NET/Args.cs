...
namespace HSNXT.ComLib.Arguments
{
    ...
    public partial class Args
    {
        ...
        public Args(string[] args, List<ArgAttribute> supported, Dictionary<string, string> named,
        ...
        /// <param name="named">Named arguments</param>
        /// <param name="positional">Positional arguments.</param>
        public Args(string[] args, string prefix, string keyValueSeparator, List<ArgAttribute> supported,
            Dictionary<string, string> named, List<string> positional)
        {
            Raw = args;
            Prefix = prefix;
        ...
        public T Get<T>(string argName, T defaultValue)
        {
            ...
            if (Named.ContainsKey(argAttrib.NameLowered))
                return GetNamedValWithDefault(argAttrib.NameLowered, defaultValue);
            return Named.ContainsKey(argAttrib.AliasLowered)
                ? GetNamedValWithDefault(argAttrib.AliasLowered, defaultValue)
                : defaultValue;
        }
        ...
        private bool IsPositionalArg(int metaIndex, string valToCheck, params string[] additionalValues)
        {
            ...
            if (val == Prefix + valToCheck || val == "-" + valToCheck || val == "--" + valToCheck ||
                val == "/" + valToCheck || val == valToCheck)
                return true;
