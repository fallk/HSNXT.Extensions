...
namespace HSNXT.ComLib.Web.HttpHandlers
{
    ...
    public class HandlerHelper
    {
        ...
        public static void SetHeaders(HttpContext context, string extensionForContent, IDictionary config,
            string configSectionName, int hashcode)
        {
            var isCachable = config == null
                ? true
                : config.GetOrDefault(configSectionName, "CacheEnabledOnClientSide", true);
            ...
            {
                var maxDaysCached =
                    config == null ? 10 : config.GetOrDefault(configSectionName, "CacheDurationInDays", 10);
