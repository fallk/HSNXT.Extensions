...
namespace HSNXT.ComLib.Lang.Parsing
{
    ...
    public class Lexer : LexerBase
    {
        ...
        private bool _hasReplacementsOrRemovals;
        ...
        private readonly Dictionary<string, Tuple<bool, string>> _inserts =
            new Dictionary<string, Tuple<bool, string>>();

        private readonly Dictionary<string, bool> _removals = new Dictionary<string, bool>();

        private static readonly IDictionary<char, bool> _opChars = new Dictionary<char, bool>
        {
            {'*', true},
            {'/', true},
            {'+', true},
        ...
        {
            _ctx = ctx;
            Init(text, '\\', new[] {SQUOTE, DQUOTE}, new[] {' ', '\t'});
        }
        ...
        /// </summary>
        public char IntepolatedStartChar
        {
            get => _interpolatedStartChar;
            set => _interpolatedStartChar = value;
        }
        ...
        /// The current token.
        /// </summary>
        public Token LastToken => _lastToken;


        /// <summary>
        /// The current token.
        /// </summary>
        public TokenData LastTokenData => _lastTokenData;


        /// <summary>
        /// The list of parsed tokens.
        /// </summary>
        public List<TokenData> ParsedTokens => _tokens;
            ...
            while (true)
            {
                ...
                if (token.Token != Tokens.WhiteSpace)
                {
                    ...
                    else if (_replacements.ContainsKey(token.Token.Text))
                    {
                        ...
                        {
                            var t = Tokens.AllTokens[replaceVal];
                            token.Token = t;
                        }
                        else
                        {
                ...
                if (last == token)
                    throw new LangException("Syntax Error", "Unexpected token", string.Empty, _pos.Line,
        ...
        public TokenData NextToken()
        {
            ...
            var line = _pos.Line;
            var tokenLength = 0;
            var cpos = _pos.LineCharPosition;
            ...
            else if (c == '"' || c == '\'')
            {
                ...
                if (_lastToken.Kind == TokenKind.Multi)
                {
                    ...
                    var text = _pos.Text.Substring(pos + 1, tokenLength);
            ...
            if ((t.Token.Kind == TokenKind.Symbol || t.Token.Type == TokenTypes.Unknown ||
                 t.Token.Type == TokenTypes.WhiteSpace) && _pos.Pos == pos)
                ReadChar();
        ...
        public TokenData PeekToken(bool allowSpace = false)
        {
            ...
            if (_pos.Pos >= _pos.Text.Length)
            {
                ...
                _endTokenData = new TokenData
                {
                    Token = Tokens.EndToken,
                    Line = _pos.Line,
                    Pos = _pos.Pos,
                    LineCharPos = _pos.LineCharPosition
                };
        ...
        {
            var text = _pos.Text;
            var currentPos = _pos.Pos + 1;

            var word = "";
            while (true && currentPos <= LAST_POSITION)
            {
                var c = text[currentPos];
                if (('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') ||
                    ('0' <= c && c <= '9') || (c == '_' || c == '-'))
                {
                    word += c;
        ...
        /// <returns></returns>
        public KeyValuePair<int, string> PeekWord(int startPos = -1, char extra1 = char.MinValue,
            char extra2 = char.MinValue)
        {
            var text = _pos.Text;
        ...
        /// <param name="extra2">A second extra character that is allowed to be part of word in addition to the allowed chars</param>
        /// <returns></returns>
        public KeyValuePair<bool, string> PeekWordWithChar(char expectChar, bool startAtNextChar,
            int maxAdvancesBeforeExpected, char extra1 = char.MinValue, char extra2 = char.MinValue)
        {
            var text = _pos.Text;
            var currentPos = startAtNextChar ? _pos.Pos + 1 : _pos.Pos;
            var found = false;
        ...
        public Token ReadString(bool handleInterpolation = true)
        {
            ...
            if (!handleInterpolation)
            {
                ...
                if (!result.Success)
                    throw new LangException("Syntax Error", "Unterminated string", string.Empty, _pos.Line,
                        _pos.LineCharPosition);
        ...
        /// <returns></returns>
        public Token ReadInterpolatedString(char quote, bool readLine = false, bool includeNewLine = false,
            bool setPositionAfterToken = true)
        {
            ...
            while (_pos.Pos <= LAST_POSITION)
            {
                ...
                // Interpolation.
                if (curr == _interpolatedStartChar && next == '{')
                {
                    // Keep track of interpolations and their start positions.
                    interpolationCount++;
                    var interpolatedStringStartPos = LineCharPos + 2;
                    var interpolatedStringLinePos = LineNumber;
                    ...
                    {
                        Token = token,
                        LineCharPos = interpolatedStringStartPos,
                        Line = interpolatedStringLinePos
                    };
        ...
        private List<TokenData> ReadInterpolatedTokens()
        {
            ...
            while (c != '}' && !IsAtEnd())
            {
                ...
                {
                    throw new LangException("syntax", "unexpected text in string", string.Empty, _pos.Line,
                        _pos.LineCharPosition);
                }
