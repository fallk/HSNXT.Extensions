...
namespace HSNXT.ComLib.Lang.Parsing
{
    ...
    public class RegisteredPlugins
    {
        ...
        // [ "monday"   ] = [ DayPlugin ]
        // [ "$IdToken" ] = [ LinqPlugin, ]
        private readonly IDictionary<string, List<ILangPlugin>> _sysStmtPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _expPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _tokPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _lexPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _postfixPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _stmtPlugins =
            new Dictionary<string, List<ILangPlugin>>();

        private readonly IDictionary<string, List<ILangPlugin>> _expStmtPlugins =
            new Dictionary<string, List<ILangPlugin>>();
        ...
        */
        ...
        private void RegisterPlugin(IDictionary<string, List<ILangPlugin>> map, ILangPlugin plugin, string[] tokens,
            bool sort)
        {
            if (tokens.Length > 0)
            {
        ...
        /// <returns></returns>
        private ILangPlugin GetPlugin(IDictionary<string, List<ILangPlugin>> map, Token token, string key = null,
            int tokenPos = 0)
        {
            // Check for end token.
