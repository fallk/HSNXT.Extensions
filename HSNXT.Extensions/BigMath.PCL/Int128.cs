...
namespace HSNXT
{
    ...
    public struct Int128 : IComparable<Int128>, IComparable, IEquatable<Int128>, IFormattable
    {
        ...
        public static int Compare(Int128 left, object right)
        {
            ...
            {
                case Int128 int128:
                    return Compare(left, int128);
                case bool b:
                    return Compare(left, new Int128(b));
                case byte b1:
                    return Compare(left, new Int128(b1));
                case char c:
                    return Compare(left, new Int128(c));
                case decimal @decimal:
                    return Compare(left, new Int128(@decimal));
                case double d:
                    return Compare(left, new Int128(d));
                case short s:
                    return Compare(left, new Int128(s));
                case int i:
                    return Compare(left, new Int128(i));
                case long l:
                    return Compare(left, new Int128(l));
                case sbyte @sbyte:
                    return Compare(left, new Int128(@sbyte));
                case float f:
                    return Compare(left, new Int128(f));
                case ushort @ushort:
                    return Compare(left, new Int128(@ushort));
                case uint u:
                    return Compare(left, new Int128(u));
                case ulong @ulong:
                    return Compare(left, new Int128(@ulong));
                case byte[] bytes when bytes.Length == 16:
                    // TODO: ensure endian.
                    return Compare(left, bytes.ToInt128());
                case Guid guid:
                    return Compare(left, new Int128(guid));
            }
        ...
        public static int Compare(Int128 left, Int128 right)
        {
            ...
            return left._hi != right._hi ? left._hi.CompareTo(right._hi) : left._lo.CompareTo(right._lo);
        }
        ...
        public static explicit operator decimal(Int128 value)
        {
            return value.Sign == 0
                ? 0
                : new decimal((int) (value._lo & 0xFFFFFFFF), (int) (value._lo >> 32), (int) (value._hi & 0xFFFFFFFF),
                    value.Sign < 0, 0);
        }
