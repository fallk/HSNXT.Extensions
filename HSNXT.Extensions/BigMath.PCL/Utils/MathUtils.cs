    ...
    public static class MathUtils
    {
        ...
        public static ulong[] ShiftRightSigned(ulong[] values, int shift)
        {
            ...
            while (shift >= valueLength)
            {
                ...
                values[length - 1] =
                    (ulong) ((long) values[length - 1] >>
                             (valueLength - 1)); //Preserve sign of upper long, will either be 0 or all f's
                shift -= valueLength;
            }
        ...
        {
            var pq256 = (Int256) pq;
            pq256.GetPrimeMultipliers(out var p256, out var q256);
            p = (Int128) p256;
            q = (Int128) q256;
        }
        ...
        private static int GetNormalizeShift(uint value)
        {
            ...
            if ((value & 0x80000000) != 0) return shift;
//            value <<= 1;
            shift += 1;
