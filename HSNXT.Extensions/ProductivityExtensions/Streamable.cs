    ...
    public static partial class Extensions
    {
        ...
        private static void OnRead(IAsyncResult ar)
        {
            ...
            if (bytesRead != 0)
            {
                ...
                state.Stream.BeginRead(state.Result.TempBuffer, 0, state.Result.Result.Length - state.Result.Length,
                    OnRead, state);
                return;
            }
        ...
        private sealed class ByteArrayAsyncResult : IAsyncResult, IDisposable
        {
            ...
            private ByteArrayAsyncResult(AsyncCallback cb, object state, byte[] buffer, int offset, byte[] tempBuffer,
                bool completed)
            {
                callback = cb;
                AsyncState = state;
