...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member 'Typeable.IsOpenGenericType(Type)'
        ...
        private static IEnumerable<TypeInfo> ByPredicate(IEnumerable<Assembly> assemblies,
            Predicate<TypeInfo> predicate)
        {
        ...
        public static IEnumerable<Type> ByImplementedInterface(this Type interfaceType, string namespaceName)
        {
            ...
            return ByPredicate(assemblies,
                type => (type.Namespace ?? string.Empty).StartsWith(namespaceName) &&
                        type.GetTypeInfo().ImplementsInterface(interfaceType));
#endif
        }
#endif
        ...
        /// <returns></returns>
        public static IEnumerable<Type> ByImplementedInterfaceInDirectory(this Type interfaceType, string directory,
            string wildcard)
        {
            if (!interfaceType.GetTypeInfo().IsInterface)
            {
        ...
        /// <param name="wildcard"></param>
        /// <returns></returns>
        public static IEnumerable<TypeInfo> ByImplementedInterfaceInDirectory(this TypeInfo interfaceTypeInfo,
        ...
        /// <returns></returns>
        public static IEnumerable<Type> ByImplementedInterfaceInDirectory(this Type interfaceType, string directory,
            string wildcard, string namespaceName)
        {
            if (!interfaceType.GetTypeInfo().IsInterface)
            {
        ...
        /// <returns></returns>
        public static IEnumerable<TypeInfo> ByImplementedInterfaceInDirectory(this TypeInfo interfaceTypeInfo,
            string directory, string wildcard, string namespaceName)
        {
            if (!interfaceTypeInfo.IsInterface)
            {
            ...
            return ByPredicate(
                System.IO.Directory.GetFiles(directory, wildcard).ToAssemblies(),
                type => (type.Namespace ?? string.Empty).StartsWith(namespaceName) &&
                        type.ImplementsInterface(interfaceTypeInfo));
        }
#endif
