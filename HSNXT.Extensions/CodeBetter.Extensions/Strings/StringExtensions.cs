namespace HSNXT
{
   ...
   {
      private static readonly Regex MultipleSpaces = new Regex(@"\s{2,}", RegexOptions.Compiled | RegexOptions.Multiline);
      private static readonly Regex NoSpaceAfterPunctuation = new Regex(@"([\.\?\!,:;\-]""?)(\S)", RegexOptions.Compiled | RegexOptions.Multiline);
      private static readonly Regex FirstLowerCaseLetterAfterSentence = new Regex(@"(\.\s*)([a-z])", RegexOptions.Compiled | RegexOptions.Multiline);
      private static readonly Regex LowercaseI = new Regex(@"([\s\p{P}])i([\s\p{P}])", RegexOptions.Compiled | RegexOptions.Multiline);
      ...
      public static string Proper(this string paragraph)
      {
         ...
         paragraph = FirstLowerCaseLetterAfterSentence.Replace(paragraph, m => m.Groups[1].Value + m.Groups[2].Value.ToUpper());
