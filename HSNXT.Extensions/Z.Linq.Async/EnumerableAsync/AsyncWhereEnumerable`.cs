...
namespace HSNXT.Linq.Async
{
    public class AsyncWhereEnumerable<T> : IEnumerable<T>
    {
        ...
        public AsyncWhereEnumerable(IEnumerable<T> source, Func<T, Task<bool>> predicate,
            CancellationToken cancellationToken)
        {
            CancellationToken = cancellationToken;
            Predicate = predicate;
        ...
        public AsyncWhereEnumerable(IEnumerable<T> source, Func<T, int, Task<bool>> predicate,
            CancellationToken cancellationToken)
        {
            CancellationToken = cancellationToken;
        ...
        public AsyncWhereEnumerable(Task<IEnumerable<T>> source, Func<T, Task<bool>> predicate,
            CancellationToken cancellationToken)
        {
            CancellationToken = cancellationToken;
            Predicate = predicate;
        ...
        public AsyncWhereEnumerable(Task<IEnumerable<T>> source, Func<T, int, Task<bool>> predicate,
            CancellationToken cancellationToken)
        {
            CancellationToken = cancellationToken;
            Predicate2 = predicate;
            ...
            if (Predicate != null)
            {
                if (SkipFilterPredicate)
                {
                    if (OrderByPredicateCompletion)
                    {
                        var enumerator2 = Source
                            .Select(x => Task.Run(() => new Tuple<T, bool>(x, Predicate(x).Result), CancellationToken))
                            .OrderByCompletion().Select(x => x.Result.Item1);
                    ...
                    else if (StartPredicateConcurrently)
                    {
                        var enumerator2 = Source
                            .Select(x => Task.Run(() => new Tuple<T, bool>(x, Predicate(x).Result), CancellationToken))
                            .ToList().Select(x => x.Result.Item1);
                        enumerator = enumerator2.GetEnumerator();
                    }
                    else
                    {
                ...
                else
                {
                    if (OrderByPredicateCompletion)
                    {
                        var enumerator2 = Source
                            .Select(x => Task.Run(() => new Tuple<T, bool>(x, Predicate(x).Result), CancellationToken))
                            .OrderByCompletion().Where(x => x.Result.Item2).Select(x => x.Result.Item1);
                        enumerator = enumerator2.GetEnumerator();
                    }
                    else if (StartPredicateConcurrently)
                    {
                        var enumerator2 = Source
                            .Select(x => Task.Run(() => new Tuple<T, bool>(x, Predicate(x).Result), CancellationToken))
                            .ToList().Where(x => x.Result.Item2).Select(x => x.Result.Item1);
                        enumerator = enumerator2.GetEnumerator();
                    }
                    else
                    {
