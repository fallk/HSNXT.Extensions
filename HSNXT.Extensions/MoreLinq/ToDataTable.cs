...
namespace HSNXT
{
    public static partial class Extensions
    {
            ...
            ICollection<Expression<Func<T, object>>> expressions)
        {
            ...
            {
                throw new ArgumentException(Resource1.One_of_the_supplied_expressions_is_not_allowed,
                    nameof(expressions), e);
            }
        }
        ...
        private static MemberInfo[] BuildOrBindSchema(DataTable table, MemberInfo[] members)
        {
            ...
            else
            {
                ...
                foreach (var info in schemas)
                {
                    ...
                    if (column == null)
                        throw new ArgumentException(string.Format(Resource1.Column_named_0_is_missing, member.Name),
                            nameof(table));

                    if (info.Type != column.DataType)
                        throw new ArgumentException(
                            string.Format(Resource1.Column_named_0_has_wrong_data_type_It_should_be_1_when_it_is_2,
                                member.Name, info.Type, column.DataType),
                            nameof(table));
