...
namespace HSNXT.Reactive.Concurrency
{
    ...
    /// </summary>
    /// <remarks>
    /// This scheduler type is typically used indirectly through the <see cref="HSNXT.Reactive.Linq.DispatcherObservable.ObserveOnDispatcher{TSource}(IObservable{TSource})"/> and <see cref="HSNXT.Reactive.Linq.DispatcherObservable.SubscribeOnDispatcher{TSource}(IObservable{TSource})"/> methods that use the Dispatcher on the calling thread.
    /// </remarks>
    public class DispatcherScheduler : LocalScheduler, ISchedulerPeriodic
    {
        ...
        public static DispatcherScheduler Instance =>
            new DispatcherScheduler(System.Windows.Threading.Dispatcher.CurrentDispatcher);
        ...
        private IDisposable ScheduleSlow<TState>(TState state, TimeSpan dueTime,
            Func<IScheduler, TState, IDisposable> action)
        {
