...
namespace HSNXT.Reactive.PlatformServices
{
    ...
    /// HSNXT.Reactive.PlatformServices assembly with your application and call <see cref="EnsureLoaded"/> during
    /// application startup to ensure enlightenments are properly loaded.
    /// </summary>
    public static class EnlightenmentProvider
    {
        /// <summary>
        /// Ensures that the calling assembly has a reference to the HSNXT.Reactive.PlatformServices assembly with
        /// platform enlightenments. If no reference is made from the user code, it's possible for the build process
        /// to drop the deployment of HSNXT.Reactive.PlatformServices, preventing its runtime discovery.
        /// </summary>
        /// <returns>
