...
namespace HSNXT.Reactive.Concurrency
{
    public static partial class Scheduler
    {
        ...
        /// <returns>The disposable object used to cancel the scheduled action (best effort).</returns>
        /// <exception cref="ArgumentNullException"><paramref name="scheduler"/> or <paramref name="action"/> is <c>null</c>.</exception>
        public static IDisposable Schedule<TState>(this IScheduler scheduler, TState state,
            Action<TState, Action<TState>> action)
        {
            ...
            return scheduler.Schedule(new Pair<TState, Action<TState, Action<TState>>> {First = state, Second = action},
                InvokeRec1);
        }

        private static IDisposable InvokeRec1<TState>(IScheduler scheduler,
            Pair<TState, Action<TState, Action<TState>>> pair)
        {
        ...
        /// <returns>The disposable object used to cancel the scheduled action (best effort).</returns>
        /// <exception cref="ArgumentNullException"><paramref name="scheduler"/> or <paramref name="action"/> is <c>null</c>.</exception>
        public static IDisposable Schedule<TState>(this IScheduler scheduler, TState state, TimeSpan dueTime,
            Action<TState, Action<TState, TimeSpan>> action)
        {
            ...
            return scheduler.Schedule(
                new Pair<TState, Action<TState, Action<TState, TimeSpan>>> {First = state, Second = action}, dueTime,
                InvokeRec2);
        }

        private static IDisposable InvokeRec2<TState>(IScheduler scheduler,
            Pair<TState, Action<TState, Action<TState, TimeSpan>>> pair)
        {
            var group = new CompositeDisposable(1);
            var gate = new object();
            var state = pair.First;
        ...
        /// <returns>The disposable object used to cancel the scheduled action (best effort).</returns>
        /// <exception cref="ArgumentNullException"><paramref name="scheduler"/> or <paramref name="action"/> is <c>null</c>.</exception>
        public static IDisposable Schedule<TState>(this IScheduler scheduler, TState state, DateTimeOffset dueTime,
            Action<TState, Action<TState, DateTimeOffset>> action)
        {
            ...
            return scheduler.Schedule(
                new Pair<TState, Action<TState, Action<TState, DateTimeOffset>>> {First = state, Second = action},
                dueTime, InvokeRec3);
        }

        private static IDisposable InvokeRec3<TState>(IScheduler scheduler,
            Pair<TState, Action<TState, Action<TState, DateTimeOffset>>> pair)
        {
            var group = new CompositeDisposable(1);
            var gate = new object();
            var state = pair.First;
