...
using HSNXT.Reactive.PlatformServices;
...
namespace HSNXT.Reactive.Concurrency
{
    public static partial class Scheduler
    {
        ...
        public static IDisposable SchedulePeriodic<TState>(this IScheduler scheduler, TState state, TimeSpan period,
            ...
            Action<TState> action)
        {
            ...
            return SchedulePeriodic_(scheduler, state, period, state_ =>
            {
                action(state_);
                return state_;
            });
        ...
        public static IDisposable SchedulePeriodic(this IScheduler scheduler, TimeSpan period, Action action)
        {
            ...
            return SchedulePeriodic_(scheduler, action, period, a =>
            {
                a();
                return a;
            });
        ...
        private static IDisposable SchedulePeriodic_<TState>(IScheduler scheduler, TState state, TimeSpan period,
            Func<TState, TState> action)
        {
            //
            // Design rationale:
...
// Workaround for WinRT not supporting <1ms resolution
        ...
        private sealed class SchedulePeriodicStopwatch<TState>
        {
            ...
            public SchedulePeriodicStopwatch(IScheduler scheduler, TState state, TimeSpan period,
                Func<TState, TState> action, IStopwatchProvider stopwatchProvider)
            {
        ...
        private sealed class SchedulePeriodicRecursive<TState>
        {
            ...
            public SchedulePeriodicRecursive(IScheduler scheduler, TState state, TimeSpan period,
                Func<TState, TState> action)
            {
                _scheduler = scheduler;
                _period = period;
