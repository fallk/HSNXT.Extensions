...
namespace HSNXT.Reactive.Concurrency
{
    ...
    {
        private static readonly Lazy<ThreadPoolScheduler> s_instance =
            new Lazy<ThreadPoolScheduler>(() => new ThreadPoolScheduler());

        private static readonly Lazy<NewThreadScheduler> s_newBackgroundThread =
            new Lazy<NewThreadScheduler>(() =>
                new NewThreadScheduler(action => new Thread(action) {IsBackground = true}));
        ...
        public override IStopwatch StartStopwatch()
        {
            ...
            // the enlightenment module to obtain the CAL, which would circle back to HSNXT.Reactive.PlatformServices
        ...
        private sealed class Timer<TState> : IDisposable
        {
            ...
            public Timer(IScheduler parent, TState state, TimeSpan dueTime,
                Func<IScheduler, TState, IDisposable> action)
            {
                _parent = parent;
                _state = state;
                ...
                finally
                {
                    ...
                    //
                    _timer = new System.Threading.Timer(this.Tick, null, dueTime,
        ...
        private sealed class PeriodicTimer<TState> : IDisposable
        {
            ...
            private void Tick(object state)
            {
                _gate.Wait(() => { _state = _action(_state); });
            }
