...
namespace HSNXT.Reactive.Linq.ObservableImpl
{
    internal static class Delay<TSource>
    {
            ...
            where TParent : Base<TParent>
        {
            ...
            internal abstract class S : _
            {
                ...
                protected Queue<HSNXT.Reactive.TimeInterval<TSource>> _queue =
                    new Queue<Reactive.TimeInterval<TSource>>();
                ...
                public override IDisposable Run(TParent parent)
                {
                    _active = false;
                    _running = false;
                    _queue = new Queue<HSNXT.Reactive.TimeInterval<TSource>>();
                    _hasCompleted = false;
                ...
                public override void OnNext(TSource value)
                {
                    ...
                    lock (_gate)
                    {
                        _queue.Enqueue(new HSNXT.Reactive.TimeInterval<TSource>(value, next));
        ...
        internal sealed class Absolute : Base<Absolute>
        {
            ...
            protected override _ CreateSink(IObserver<TSource> observer, IDisposable cancel) =>
                _scheduler.AsLongRunning() != null
                    ? (Base<Absolute>._) new L(this, observer, cancel)
                    : new S(this, observer, cancel);
                    ...
                    lock (_gate)
                    {
                        ...
                        if (oldQueue.Count > 0)
                        {
                            ...
                            while (oldQueue.Count > 0)
                            {
                                ...
                                _queue.Enqueue(
                                    new Reactive.TimeInterval<TSource>(item.Value, item.Interval.Add(_delay)));
                            }
        ...
        internal sealed class Relative : Base<Relative>
        {
            ...
            protected override _ CreateSink(IObserver<TSource> observer, IDisposable cancel) =>
                _scheduler.AsLongRunning() != null
                    ? (Base<Relative>._) new L(this, observer, cancel)
                    : new S(this, observer, cancel);
    ...
    internal static class Delay<TSource, TDelay>
    {
        ...
        internal class Selector : Base<Selector>
        {
            ...
            protected override Base<Selector>._ CreateSink(IObserver<TSource> observer, IDisposable cancel) =>
                new _(_delaySelector, observer, cancel);
        ...
        internal sealed class SelectorWithSubscriptionDelay : Base<SelectorWithSubscriptionDelay>
        {
            ...
            public SelectorWithSubscriptionDelay(IObservable<TSource> source, IObservable<TDelay> subscriptionDelay,
            ...
            protected override Base<SelectorWithSubscriptionDelay>._ CreateSink(IObserver<TSource> observer,
                IDisposable cancel) => new _(_delaySelector, observer, cancel);
            ...
            new private sealed class _ : Base<SelectorWithSubscriptionDelay>._
            {
                ...
                protected override IDisposable RunCore(SelectorWithSubscriptionDelay parent)
                {
                    ...
                    subscription.Disposable =
                        parent._subscriptionDelay.SubscribeSafe(
                            new SubscriptionDelayObserver(this, parent._source, subscription));
