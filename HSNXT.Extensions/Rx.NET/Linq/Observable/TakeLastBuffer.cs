...
namespace HSNXT.Reactive.Linq.ObservableImpl
{
    internal static class TakeLastBuffer<TSource>
    {
        ...
        internal sealed class Time : Producer<IList<TSource>, Time._>
        {
            ...
            protected override _ CreateSink(IObserver<IList<TSource>> observer, IDisposable cancel) =>
                new _(_duration, observer, cancel);
            ...
            {
                private readonly TimeSpan _duration;
                private Queue<HSNXT.Reactive.TimeInterval<TSource>> _queue;
                ...
                public void OnNext(TSource value)
                {
                    ...
                    _queue.Enqueue(new HSNXT.Reactive.TimeInterval<TSource>(value, now));
                    Trim(now);
                }
