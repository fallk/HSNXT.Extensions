...
namespace HSNXT
{
    public static class ByteSizeFriendlyName
    {
        ...
        {
            if (bytes < 0)
                throw new ArgumentOutOfRangeException(nameof(bytes), "bytes cannot be a negative value");

            options = options ?? FriendlyNameOptions.Default;

            var friendlySize = GetFriendlySize(bytes, out var units);

            var sizeFormat = (options.GroupDigits ? "#,0." : "0.") + new string('#', options.DecimalPlaces);
            var size = friendlySize.ToString(sizeFormat);

            switch (options.UnitDisplayMode)
            {
                case UnitDisplayMode.AlwaysDisplay:
                    return $"{size} {units}";
                case UnitDisplayMode.AlwaysHide:
                    return size;
            }

            return bytes < 1024 ? size : $"{size} {units}";
        }

        private static double GetFriendlySize(long bytes, out string units)
        {
            foreach (var byteMapping in ByteMappings)
            {
