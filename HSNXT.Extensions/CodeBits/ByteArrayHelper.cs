    ...
    public static partial class Extensions
    {
        ...
        public static bool IsEqualTo(this byte[] source, params byte[] other)
        {
            ...
            if (source.Length != other.Length)
                return false;
            return !source.Where((t, byteIdx) => t != other[byteIdx]).Any();
        }
        ...
        {
            var sequenceIndex = IndexOfSequence(source, start, source.Length - start + 1, sequence);
            if (sequenceIndex == -1)
                return null;
        ...
        {
            var sequenceIndex = 0;
            var endIndex = Math.Min(source.Length, start + count);
            for (var byteIdx = start; byteIdx < endIndex; byteIdx++)
            {
        ...
        public static int[] IndexOfSequences(this byte[] source, int start, int count, byte[] sequence)
        {
            ...
            var sequenceLocation = IndexOfSequence(source, start, count, sequence);
        ...
        public static byte[][] SplitBySequence(this byte[] source, int start, int count, byte[] sequence)
        {
            ...
            {
                var startIndex = locationIdx > 0 ? locations[locationIdx - 1] + sequence.Length : start;
                var endIndex = locations[locationIdx] - 1;
