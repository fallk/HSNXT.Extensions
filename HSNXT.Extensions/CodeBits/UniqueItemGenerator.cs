    ...
    public static class UniqueItemGenerator
    {
            ...
            string duplicateItemFormat, StringComparison comparison)
        {
            ...
            if (duplicateItemFormat == null)
                throw new ArgumentNullException(nameof(duplicateItemFormat));

            //If the items collection is empty, return itemName
            var en = items as string[] ?? items.ToArray();
            if (!en.Any())
                return itemName;

            var eligibleItems = en.Where(item => item.StartsWith(itemName, comparison));
            var en2 = eligibleItems as string[] ?? eligibleItems.ToArray();
            if (!en2.Any(item => item.Equals(itemName, comparison)))
                return itemName;

            for (var i = 1; i < int.MaxValue; i++)
            {
                var newItemName = string.Format(duplicateItemFormat, itemName, i);
                if (!en2.Any(item => item.Equals(newItemName, comparison)))
                    return newItemName;
            }
