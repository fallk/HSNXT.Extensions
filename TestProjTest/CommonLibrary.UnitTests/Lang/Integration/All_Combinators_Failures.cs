...
namespace HSNXT.ComLib.Lang.Tests.Integration
{
    ...
    public class Plugin_Integration_Failures : ScriptTestsBase
    {
        ...
        public void Can_Use_Linq_Plugin_Inside_Parenthesis()
        {
            ...
            {
                new Tuple<string, Type, object, string>("i", typeof(double), 1,
                    items +
                    " var i = 0; var favs = (items where item.pages < 180);                            if(favs.length == 3 && favs[0].pages == 100) i = 1;"),
                new Tuple<string, Type, object, string>("i", typeof(double), 1,
                    items +
                    " var i = 0; var favs = (items where item.pages < 180 && item.author == 'kdog');   if(favs.length == 2 && favs[1].pages == 140) i = 1;"),
                new Tuple<string, Type, object, string>("i", typeof(double), 1,
                    items +
                    " var i = 0; var favs = (from book in items where book.pages < 180 && book.author == 'kdog');   if(favs.length == 2 && favs[1].pages == 140) i = 1;")
            };
        ...
        public void Can_Use_Linq_Plugin_Inside_Parenthesis_In_If()
        {
            ...
            {
                new Tuple<string, Type, object, string>("i", typeof(double), 1,
                    items + " var i = 0; if( (items where item.pages == 120)[0].author == 'kdog' ) i = 1;"),
                new Tuple<string, Type, object, string>("i", typeof(double), 1,
                    items +
                    " var i = 0; if( (items where item.pages < 200 and item.author is 'amycat')[0].pages == 100 ) i = 1;")
            };
