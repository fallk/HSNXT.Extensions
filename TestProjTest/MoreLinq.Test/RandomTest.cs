namespace HSNXT.Test
{
    ...
    public class RandomTest
    {
        ...
        {
            var resultA = Extensions.RandomDouble().Take(RandomTrials);
            var resultB = Extensions.RandomDouble(new Random()).Take(RandomTrials);
        ...
        {
            const int max = 100;
            var resultA = Extensions.Random(max).Take(RandomTrials);
            var resultB = Extensions.Random(new Random(), max).Take(RandomTrials);
        ...
        public void TestRandomMinMaxConstraint()
        {
            const int min = 0;
            const int max = 100;
            var resultA = Extensions.Random(min, max).Take(RandomTrials);
            var resultB = Extensions.Random(new Random(), min, max).Take(RandomTrials);
        ...
        public void TestRandomEquivalence()
        {
            ...
            var randomSeq = Extensions.Random(randB);
